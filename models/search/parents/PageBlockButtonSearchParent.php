 ...
 */
class PageBlockButtonSearchParent extends ModelSearch {
    public static $main_model_class = '\\app\\models\\PageBlockButton';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['title', 'link', 'page_block_id'], 'string'],
            [['color'], 'string', 'max' => 50],
            [['public'], 'number'],
            [['weight'], 'integer'],
        ]);
    ...
    public function field_filter_input($attribute) {
        ...
        if ($attribute == 'color') return $this->filter_select($attribute);
    ...
    public function search_field_color($query, $parent_atttribute = '') {
        $this->search_type_select($query, 'color', $parent_atttribute);
    }
