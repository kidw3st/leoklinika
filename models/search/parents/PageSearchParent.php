 ...
 */
class PageSearchParent extends ModelSearch {
    public static $main_model_class = '\\app\\models\\Page';
    ...
    public function rules()
    {
        return array_merge(parent::rules(), [
            [['title', 'url', 'button_text', 'seo_title', 'seo_description', 'seo_keywords', 'og_title', 'og_description', 'created_at', 'updated_at'], 'string'],
            [['show_header', 'public'], 'number'],
            [['template', 'form'], 'string', 'max' => 50],
        ]);
    }
    ...
    public function field_filter_input($attribute) {
        ...
        if ($attribute == 'show_header') return $this->filter_checkbox($attribute);
        if ($attribute == 'template') return $this->filter_select($attribute);
        if ($attribute == 'button_text') return $this->filter_string($attribute);
        if ($attribute == 'form') return $this->filter_select($attribute);
    ...
    public function search_field_show_header($query, $parent_atttribute = '') {
        $this->search_type_checkbox($query, 'show_header', $parent_atttribute);
    }
    public function search_field_template($query, $parent_atttribute = '') {
        $this->search_type_select($query, 'template', $parent_atttribute);
    }
    public function search_field_button_text($query, $parent_atttribute = '') {
        $this->search_type_string($query, 'button_text', $parent_atttribute);
    }
    public function search_field_form($query, $parent_atttribute = '') {
        $this->search_type_select($query, 'form', $parent_atttribute);
    }
