...
<!--                    <button class="block-content__btn btn btn_primary _open-popup" data-target-id="appointment_service_detail_--><?php //=$service->id?><!--">Записаться на приём</button>-->
            ...
            <?php if (!empty($service->links_real)) { ?>
                <section class="actions">
                    ...
                    <div class="container">
                        <?php foreach ($service->links_real as $link) { ?>
                            <a href="<?=$link->link?>" class="actions__item actions__item_<?=$link->background_color?>" style="background-image: url('<?=$link->image_obj->getImage()?>');">
                                <div class="actions__item__text-wrapper">
                                    <p class="actions__item__title megatext_s megatext_s_strong"><?=$link->title?></p>
                                    <?php if (!empty($link->link)) { ?>
                                        <p href="<?=$link->link?>" class="actions__item__pseudolink btn btn_link_arrow<?=$link->background_color=='dark'?' btn_link_arrow_white':''?>"><?=(!empty($link->link_title)?$link->link_title:'Подробнее')?></p>
                                    <?php } ?>
                                </div>
                            </a>
                        <?php } ?>
