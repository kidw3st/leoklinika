<div class="form-group<?=$model->hasErrors($attribute)?' has-error':''?>">
    ...
    <textarea id="<?=$attribute?>" name="<?=$model->formNameWithVar($attribute)?>" class="form-control" rows="1" style="resize: none;" placeholder="<?=$model->getAttributeHint($attribute)?>"><?=$model->{$attribute}?></textarea>
