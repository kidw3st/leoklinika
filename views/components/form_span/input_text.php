...
<label class="form__label form__label_<?=$attribute?><?=$model->hasErrors($attribute)?' _error':''?><?=(!empty($model->$attribute) && !$model->hasErrors($attribute))?' _success':''?>">
    ...
    <input class="form__input bodytext_m<?=($attribute=='name')?' fio-mask':''?>" size="40" name="<?=$model->formNameWithVar($attribute)?>" type="text" placeholder="<?=$model->getAttributeHint($attribute)?>" value="<?=$model->$attribute?>"<?=$option_attributes?>>
</label>